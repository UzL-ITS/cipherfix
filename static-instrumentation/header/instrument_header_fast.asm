...
%define MASK_BUFFER_OFFSET -0x40000000
...
.allocate_stack_mask_buffer:
	...
	; Allocate mask buffer for stack memory block
	push rsi
	call allocate_mask_buffer
	pop rsi
...
.next_block:
	...
	; Get address of mask buffer
	add rdi, MASK_BUFFER_OFFSET
...
.continue_realloc:
	...
	; r11 holds the old allocation tracker value
...
.call_realloc:
	...
	; Ensure that r11 doesn't get clobbered
	mov rax, rbx
	mov rbx, r11
...
.new_mask:
	...
	.copy_with_random_mask:
		...
		.copy_with_random_mask_loop:
			...
			; Read old data
			mov rax, qword [r12+rcx] ; Address of secondary data buffer
			
			; Apply old mask
			xor rax, qword [rbp+rcx] ; Address of secondary mask buffer
	...
	.copy_with_zero_mask:
		...
		; Fill underlying mask buffer with zeroes
		xor eax, eax
...
; Allocates a new mask buffer for the given memory block.
; The new mask buffer is not initialized.
; Parameters:
; - rdi: Memory block begin address (multiple of page size).
; - rsi: Memory block size (multiple of page size).
; Returns the address of the mask buffer.
[global allocate_mask_buffer]
allocate_mask_buffer:
