...
void dump_secret(u8 *data, int length, const char* prefix);
...
int crypto_hashblocks(u8 *x,const u8 *m,u64 n)
{
  ...
  while (n >= 128) {
    ...
    FOR(i,80) {
      //dump_secret((u8*)a, 64, "a-pre");
      FOR(j,8) b[j] = a[j];
      t = a[7] + Sigma1(a[4]) + Ch(a[4],a[5],a[6]) + K[i] + w[i%16];
      b[7] = t + Sigma0(a[0]) + Maj(a[0],a[1],a[2]);
      b[3] += t;
      FOR(j,8) a[(j+1)%8] = b[j];
      //dump_secret((u8*)a, 64, "a-post");
      if (i%16 == 15)
	FOR(j,16)
...
int crypto_hash(u8 *out,const u8 *m,u64 n)
{
  ...
  //dump_secret(h, 64, "h1");
  crypto_hashblocks(h,m,n); // int crypto_hashblocks(u8 *x,const u8 *m,u64 n)
  //dump_secret(h, 64, "h2");
  m += n;
  n &= 127;
  m -= n;
  ...
  x[n-9] = b >> 61;
  ts64(x+n-8,b<<3);
  //dump_secret(x, 256, "x2");
  //dump_secret(h, 64, "h3");
  crypto_hashblocks(h,x,n); // int crypto_hashblocks(u8 *x,const u8 *m,u64 n)
  //dump_secret(h, 64, "h4");
...
sv scalarbase(gf p[4],const u8 *s)
{
  ...
  set25519(q[1],Y);
  set25519(q[2],gf1);
  //dump_secret((u8 *)q, sizeof(q), "q-1");
  M(q[3],X,Y);
  scalarmult(p,q,s);
  //dump_secret((u8 *)p, sizeof(q), "p-1");
  //dump_secret((u8 *)q, sizeof(q), "q-2");
}
...
int crypto_sign(u8 *sm,u64 *smlen,const u8 *m,u64 n,const u8 *sk)
{
  ...
  *smlen = n+64;
  ...
  scalarbase(p,r);
  //dump_secret(r, 64, "r3");
  //dump_secret(sm, 64, "sm2");
  //dump_secret((u8*)p, sizeof(p), "p1");
  pack(sm,p);
  //dump_secret(sm, 64, "sm3");
  //dump_secret((u8*)p, sizeof(p), "p2");
