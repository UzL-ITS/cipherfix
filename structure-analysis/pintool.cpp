...
public:
    ...
    std::unordered_set<uintptr_t> InstructionsExecutedSinceLastWrite;
...
std::unordered_map<REG, int, RegHash> _registerCounts;

// Mapping of Pin registers to the Registers enum.
std::map<REG, Registers> _registerMapping;

// Instruction info.
std::unordered_map<UINT64, InstructionData*> _instructionData;
...
#define CREATE_REG_STATE(reg) new RegisterState(Registers::reg, REG_StringShort(REG_ ## reg))
RegisterState* const _registerTrackingStates[] = {
    CREATE_REG_STATE(RAX),
    CREATE_REG_STATE(RBX),
    CREATE_REG_STATE(RCX),
    CREATE_REG_STATE(RDX),
    CREATE_REG_STATE(RDI),
    CREATE_REG_STATE(RSI),
    CREATE_REG_STATE(RBP),
    CREATE_REG_STATE(RSP),
    CREATE_REG_STATE(R8),
    CREATE_REG_STATE(R9),
    CREATE_REG_STATE(R10),
    CREATE_REG_STATE(R11),
    CREATE_REG_STATE(R12),
    CREATE_REG_STATE(R13),
    CREATE_REG_STATE(R14),
    CREATE_REG_STATE(R15),
    CREATE_REG_STATE(YMM0),
    CREATE_REG_STATE(YMM1),
    CREATE_REG_STATE(YMM2),
    CREATE_REG_STATE(YMM3),
    CREATE_REG_STATE(YMM4),
    CREATE_REG_STATE(YMM5),
    CREATE_REG_STATE(YMM6),
    CREATE_REG_STATE(YMM7),
    CREATE_REG_STATE(YMM8),
    CREATE_REG_STATE(YMM9),
    CREATE_REG_STATE(YMM10),
    CREATE_REG_STATE(YMM11),
    CREATE_REG_STATE(YMM12),
    CREATE_REG_STATE(YMM13),
    CREATE_REG_STATE(YMM14),
    CREATE_REG_STATE(YMM15)
};

// Current state of flag tracking.
#define CREATE_FLAG_STATE(flag) new FlagState(StatusFlags::flag, #flag)
FlagState* const _flagTrackingStates[] = {
    CREATE_FLAG_STATE(C),
    CREATE_FLAG_STATE(P),
    CREATE_FLAG_STATE(A),
    CREATE_FLAG_STATE(Z),
...
VOID InstrumentTrace(TRACE trace, VOID* v)
{
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            ...
            // Manually add second read register for cmov instructions
            // Pin only detects 'cmov <write>, <read>', but the destination register may be left unchanged
            if (INS_IsPredicated(ins) && IsCmov(INS_Opcode(ins)))
            {
                instructionData->ReadRegisters |= instructionData->WriteRegisters;
            }
...
// Helper function to detect cmov instructions.
inline bool IsCmov(OPCODE opcode)
{
    switch (opcode)
    {
